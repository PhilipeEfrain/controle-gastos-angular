...
2. **Auditoria de Quotas e Consumo do Firebase**:
   - Auditar nos testes unitários e de integração se nenhuma ação do usuário dispara chamadas redundantes ao Firestore (`getDocs`, `onSnapshot`).
   - Garantir que testes BDD verificam explicitamente que o componente cancela suas subscrições ao ser destruído via `takeUntilDestroyed(this.destroyRef)` ou `destroyRef.onDestroy`.
   - Validar se estratégias de cache (ex: `syncedMonths`, Signals memoizados) estão ativas e funcionando.
3. **Testes E2E e Validação Visual no Navegador (Browser Harness)**:
...
4. **Pareceres Formais no Pull Request (`[QA Sign-off]` e `[Cost/Quota Sign-off]`)**:
